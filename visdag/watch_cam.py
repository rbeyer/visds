...
def to_db(args, processor=None):
    ...
    if processor is None:
        client, processor = spin_processor(args)
...
def to_outdir(args, processor=None):
    if isinstance(args, dict):
        outdir=Path(args["outdir"])
    else:
        outdir = args.outdir
...
def to_json(args, processor=None): 
    watched = Watcher() 
    subscription = processor.create_parameter_subscription(
            parameters, 
...
def main():
    ...
    if args.dburl is not None:
        to_db(args, processor)

    elif args.outdir is not None:
        to_outdir(args, processor)

    elif args.json is not None:
        to_json(args, processor)
